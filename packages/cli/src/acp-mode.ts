...
export async function startAcpMode(options: AcpModeOptions = {}): Promise<AcpModeHandle> {
   ...
   * > 配置 `dataDir` > `<workdir>/.fengagent-cordis`。前者是 Multica 守护进程判定
   * `session_home_reachable` / `resume_reachable` 的依据：会话库不落在它指定的仓里，
   * 下一轮守护进程就会丢掉前会话，`session/resume` 永远走不到（AGE-29 的 A 项欠账）。
   *
