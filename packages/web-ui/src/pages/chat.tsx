              ...
              <div className="settings-menu" role="menu" aria-label="设置">
                ...
                <div className="settings-menu__group">
                  ...
                  {session.activeSession && (
                    ...
                    >
                      <span className="settings-menu__text">对话图面板</span>
                      <span className={`settings-menu__toggle ${showGraph ? "settings-menu__toggle--on" : ""}`}>
                        {showGraph ? "开" : "关"}
                      </span>
                    </button>
                  )}
...
/**
 * 权限请求卡片 — human-in-the-loop 改参审批。
 *
 * 入参 JSON 可编辑：修改后点「Allow 执行」会以修改后的参数提交（allow + input），
 * 工具将按新参数执行；不改参数直接 Allow 等价于原参数放行。Deny 拒绝本次调用。
 */
