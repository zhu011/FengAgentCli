...
- **Multica 里「准备会话」一直不启动 / 报「hermes initialize failed: hermes process exited」** — Multica 守护进程把本地运行时当**子进程**拉起，走 stdin/stdout 上的 ACP（Agent Client Protocol）JSON-RPC；此前 `fengagent acp` 只监听 HTTP + SSE 且从不读 stdin，握手无从完成，宿主只能看到子进程退出：
  - `fengagent acp` 默认改为 **stdio JSON-RPC**：`initialize` / `authenticate` / `session/new` / `session/prompt` / `session/cancel`（`session/set_model` 容错支持），方法名、字段形状、JSON-RPC 错误码与措辞严格对标同族 `dsh-acp`（`packages/server/src/acp-stdio.ts`，main 适配自 refactor）；
