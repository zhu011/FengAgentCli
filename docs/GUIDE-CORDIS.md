...
3.5. [全局安装（可选）](#35-全局安装可选安装后任意目录直接-fengagent)
4. [安装依赖](#4-安装依赖)
...
> WebUI 中：打开「对话图」面板，点任意活跃节点（用户 / 助手 / 工具）上的
> 「**回退并重答**」按钮，效果与 CLI `/rollback <节点id>` 一致——回退到该轮提问处、
> 截断会话并**自动重新回答**，新回答挂在分支点下，旧分支灰显保留（可溯源）。

![回退前](site/screenshots/13-graph-before-rollback.png)  ![回退后](site/screenshots/14-graph-after-rollback.png)

### 10.1 更细的三种操作（图面板）

回退默认是**轮级**的（回到该轮提问处、整轮重来）。图面板在此之上多了三种更细的操作：

| 操作 | 在哪点 | 效果 |
|------|--------|------|
| **从此步续跑** | 助手步骤节点 | 回退点精确到**一步**（而不是整轮）：该步之前已经执行过的工具**不会重复执行**，AI 从这一步接着往下做 |
| **改参** | 助手步骤下的某个工具调用 | 展开入参 → 改成新参数 → **改参并重放**：该工具以**新入参**重新执行（其余步骤照旧） |
| **✏️ 已改参** | 节点上的标记 | 该步的工具入参被用户改过。鼠标悬停可以看到**改参前后**的原始参数与实际参数 |

"改参并重放"和审批弹窗里的"修改参数后放行"走的是同一条链路，所以无论从哪个入口改的参，
图上都会留下同样的 ✏️ 标记，事后可追溯。

> 轮级回退的语义没有变化：点节点上的「回退并重答」仍然是回到该轮提问处整轮重来。
